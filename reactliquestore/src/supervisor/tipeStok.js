...
export default function ReviewStok() {
  ...
  if (!nama || nama.length > 255) {
    tempErrors.nama = 'Nama barang harus diisi';
  } else if (nama.length > 255){
    tempErrors.nama = 'Nama barang maksimal 255 karakter'
  }
  // Validasi Varian
  if (!varian || varian.length > 255) {
  ...
  } else if (!integerRegex.test(weight)) {
    tempErrors.weight = 'Berat barang harus berupa angka';
  ...
  } else if (!integerRegex.test(capitalprice)) {
    tempErrors.capitalprice = 'Harga modal barang harus berupa angka';
  ...
  } else if (!integerRegex.test(defaultprice)) {
    tempErrors.defaultprice = 'Harga jual barang harus berupa angka';
  ...
  const fetchDataInventori = async () => {
    try {
      const response = await axios.get('http://localhost:8080/dataTipe');
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      try {
        const response = await axios.post('http://localhost:8080/tambahTipe', { nama, varian, weight, capitalprice, defaultprice });
        console.log(response.data);
        setOpenTambah(false);
        ...
        setShowError(true);
        setMessageInsert("Gagal Menambah Tipe Barang");
        setTimeout(() => {
          setShowError(false);
        }, 5000);
      }
  ...
  const handleConfirmEdit = async (e) => {
    ...
    if (validate()) {
      ...
      const id = updatedId;
      try {
        const response = await axios.post('http://localhost:8080/editTipe', { id, nama, varian, weight, capitalprice, defaultprice });
        console.log(response.data);
        setOpenEdit(false);
  ...
  const handleConfirmDelete = async (e) => {
    ...
    try {
      const id = updatedId;
      const response = await axios.delete(`http://localhost:8080/deleteTipe/${id}`);
      console.log(response.data);
                  ...
                  <TableBody>
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell align="center">{formatCurrency(row.capitalPrice)}</TableCell>
                          <TableCell align="center">{formatCurrency(row.defaultPrice)}</TableCell>
